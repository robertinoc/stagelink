...
import type { ArtistRelease, ReleasesBlockConfig } from '@stagelink/types';
import { ReleaseCoverImage } from '../../public-page/components/ReleaseCoverImage';

interface ReleasesBlockRendererProps {
  title: string | null;
...
function extractYear(releaseDate: string | null): string | null {
  ...
  const match = releaseDate.trim().match(/^(\d{4})/);
  return match ? match[1]! : null;
}

/**
 * Public renderer for the "Releases" block. Mirrors the visual language of the
 * legacy auto ReleasesSection but as a client component (block renderers run
 * inside PublicPageClient). The full release objects are resolved server-side
 * (localizeBlock) and arrive on `config.releases`.
 */
export function ReleasesBlockRenderer({ title, config }: ReleasesBlockRendererProps) {
  const t = useTranslations('public_page.releases');
  const releases: ArtistRelease[] = config.releases ?? [];
