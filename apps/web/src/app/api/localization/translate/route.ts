...
export async function POST(request: Request) {
      ...
      input: [
        {
          ...
          content: [
            {
              type: 'input_text',
              text: `Translate structured artist-facing product content from ${sourceLocale} to ${targetLocale}. Preserve artist names, usernames, URLs, emails, genres, brand names, and formatting. Return natural copy for StageLink users. Output valid JSON only.`,
            },
          ],
        },
        {
  ...
  try {
    const parsedTranslations = z.record(z.string()).parse(JSON.parse(outputText));
