...
| Umami                 | StageLink Platform analytics when configured                           | Consent-gated platform page views, UTM parameters, and `platform_*` events           | Processor if configured as service provider                                        | Region, DPA, retention, and IP handling need confirmation              | Active for platform when public env vars are set |
