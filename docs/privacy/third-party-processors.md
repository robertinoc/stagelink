...
| Umami                 | Behind the Stage product/admin analytics when configured               | Behind page views, referrers, device context, and `behind_*` events                  | Processor if configured as service provider                                        | Region, DPA, retention, and IP handling need confirmation              | Active for Behind when public env vars are set |
...
- keep Umami scoped to Behind operator/product analytics only;
