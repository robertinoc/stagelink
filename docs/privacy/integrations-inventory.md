...
| Umami                      | Active for Behind when public env vars are set | Internal product/admin analytics                         | Behind page views and `behind_*` events                                   | analytics dashboards/events                              | analytics event copies in provider                     | Processor if configured under DPA/service-provider terms                                              | Medium |
...
- Umami is mounted only in Behind the Stage and remains excluded from public
  artist pages, artist dashboards, auth, landing, and marketing routes.
...
| --------------- | ---------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| Analytics stack | PostHog active if configured; Umami active for Behind when public env vars are set | Confirm provider regions, retention, IP handling, and public policy wording         |
