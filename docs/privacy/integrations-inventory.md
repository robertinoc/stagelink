...
| Umami                      | Active for StageLink Platform when public env vars are set | Product and growth analytics                             | Consent-gated platform pageviews, UTM parameters, and `platform_*` events | analytics dashboards/events                              | analytics event copies in provider                     | Processor if configured under DPA/service-provider terms                                              | Medium |
