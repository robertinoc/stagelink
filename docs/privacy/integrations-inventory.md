...
| Umami | Not active in code | Analytics | none currently | none currently | none currently | Processor if enabled later | Medium |
...
- Umami is not active in the audited source and must remain out of public
  policy claims unless it is later enabled.
...
| --- | --- | --- |
| Analytics stack | PostHog active if configured; Umami not active | Confirm final stack and hosting region; update public policies |
