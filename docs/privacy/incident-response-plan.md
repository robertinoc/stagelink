...
| Analytics providers     | PostHog and Behind-only Umami                                   | StageLink controls event selection, route scoping, and consent where applicable; providers process ingested events.                   | If event payload overexposes data, disable event/provider, request provider-side deletion, assess consent and breach impact.                          |
