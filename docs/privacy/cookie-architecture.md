...
| Analytics | Product and public-page analytics, artist dashboard metrics, PostHog, StageLink Platform Umami | `sl_consent`, `sl_ac`, PostHog browser identifiers after opt-in, timestamp-only signup conversion marker in `sessionStorage` after opt-in, public page/link/smart-link analytics events, Umami script/runtime storage on `stagelink.art` | Consent for public/artist-facing and platform analytics | Yes              |
