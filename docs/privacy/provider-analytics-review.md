...
| Umami                      | StageLink Platform analytics when `NEXT_PUBLIC_UMAMI_PLATFORM_WEBSITE_ID` is configured | Consent-gated platform page views, UTM parameters, and `platform_*` product events         | Mounted on platform layout, domain allowlisted to `stagelink.art`, DNT respected, no PII event properties | Medium |
