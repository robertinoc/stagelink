...
| Umami                    | Behind product analytics   | Behind page views, referrers, device context, `behind_*` events | Processor if configured under DPA/service-provider terms                   | Evidence needed for active Behind setup                      | Confirm region/transfers              | Medium | Confirm Cloud/self-host region, DPA, retention, IP handling, DNT behavior  |
