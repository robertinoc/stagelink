...
contract review, or counsel review. "Evidence needed" means the provider should
not be treated as fully launch-cleared until StageLink stores the relevant DPA,
SCC, region, retention, and subprocessor evidence.
...
| Umami | Analytics future | web analytics | Processor if enabled | Not active; evidence needed only if enabled | TBD | Medium | Keep disabled or review before enabling |
