...
| Umami                    | StageLink Platform analytics | Consent-gated platform page views, UTM parameters, `auth_*` events | Processor if configured under DPA/service-provider terms                   | Evidence needed for active platform setup                    | Confirm region/transfers              | Medium | Confirm Cloud/self-host region, DPA, retention, IP handling, DNT behavior  |
